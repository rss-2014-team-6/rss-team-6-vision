 ...
 * 
 *         Apply a gaussian filter of size 5x5, with sigma=1. <br>
 *         The 2D filter:
 * 
 *         <pre>
 *         [  0.7  3.3  5.5  3.3  0.7 ]
...
public class GaussianBlur {
    /**
     * Apply a 5x5 gaussian blur of sigma = 1. Both images must have the same
     * size, defined by the width and height.
     * 
     * @param srcpix
     *            The source image.
     * @param dstpix
    ...
    public static void apply(byte[] srcpix, byte[] dstpix, int w, int h) {
        ...
        if (srcpix.length != w * h * 3) {
            throw new IllegalArgumentException(
                    "Size of image does not match width and height!");
        }
    ...
    private static void applyBoxHelper(byte[] srcpix, byte[] dstpix, int w,
            int h) {
        byte[] tmppix = new byte[w * h * 3];
